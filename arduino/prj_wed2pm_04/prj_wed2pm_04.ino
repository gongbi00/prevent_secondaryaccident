...
//  //sendData("AT+CIPCLOSE\r\n",1000,DEBUG);                                   //연결종료(respose:CLOSED OK)
      ...
      if(Serial1.find("CAR-0")){
          sendData("find CAR-01",1000,DEBUG);
...
void Init_wifi(void){
  ...
  sendData("AT+CWMODE=3\r\n",10000,DEBUG);                             // configure as access point, <MODE_1=station, 2=AP, 3=station+AP>
  sendData("AT+CWSAP=\"CAR-02\",\"1234test\",11,3\r\n",10000,DEBUG);   // join the access point <"ssid","pwd","chl","ecn"> <ecn_0:open, 1:wpa_psk, 2:wpa2_psk, 3:wpa_wpa2__psk>
  sendData("AT+CIPMUX=1\r\n",10000,DEBUG);                             // configure for multiple connections, <0:single, 2:multiple connnection>
  sendData("AT+CIPAP=\"192.168.2.1\",\"192.168.2.1\",\"255.255.255.0\r\n",1000,DEBUG); // configure for multiple connections, <0:single, 2:multiple connnection>
